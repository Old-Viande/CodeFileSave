...
public class TransformTweenDrawer : PropertyDrawer
{
    ...
    public override void OnGUI (Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        SerializedProperty startAimPro = property.FindPropertyRelative("startAim");
        SerializedProperty EndAimPro = property.FindPropertyRelative("endAim");
        ...
        EditorGUI.PropertyField(singleFieldRect, startAimPro);

        if (startAimPro.enumValueIndex == (int)TransformTweenBehaviour.AimPosition.relayPoint)
        {
            SerializedProperty relayhightProp = property.FindPropertyRelative("relayhight");
            singleFieldRect.y += EditorGUIUtility.singleLineHeight;

            EditorGUI.PropertyField(singleFieldRect, relayhightProp);
        }
        singleFieldRect.y += EditorGUIUtility.singleLineHeight;

        EditorGUI.PropertyField(singleFieldRect, EndAimPro);

        if (EndAimPro.enumValueIndex == (int)TransformTweenBehaviour.AimPosition.relayPoint)
        {
            SerializedProperty relayhightProp = property.FindPropertyRelative("relayhight");
            singleFieldRect.y += EditorGUIUtility.singleLineHeight;
