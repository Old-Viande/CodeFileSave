...
public class CreateManager : Singleton<CreateManager>
{
    ...
    public void EnenmyCreate(Vector3 position)
    {
        ...
        enemyList.Add(Instantiate(enemies.enemies[0].Item, position, Quaternion.identity));//加入数组，进行动态变化
        enemyList[enemyList.Count - 1].name = enemies.enemies[0].name + "-" + enemyList.Count;        //每生成一个，数组长度都会增加，数组长度-1得到的是目前数组最新存入的那个单位
        GridManager.Instance.pathFinder.GetGrid().GetValue(position).canWalk = false;   //把生成位置的格子设为不可行走
    }

    public void EnenmyCreate(string name, Vector3 position)
    {
        Enemy original = null;
        for (int i = 0; i < enemies.enemies.Count; i++)
        {
            if (enemies.enemies[i].name == name)
            {
                original = enemies.enemies[i];
                break;
            }
        }
        if (original == null)
        ...
        enemyList.Add(Instantiate(original.Item, position, Quaternion.identity));//加入数组，进行动态变化
        enemyList[^1].name = original.name + "-" + enemyList.Count;        //每生成一个，数组长度都会增加，数组长度-1得到的是目前数组最新存入的那个单位
        GridManager.Instance.pathFinder.GetGrid().GetValue(position).canWalk = false;   //把生成位置的格子设为不可行走
    }

    public void EnemyCreateInTheRoom(Vector3 roomPosition)
    {
